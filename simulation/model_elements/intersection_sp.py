...
class Intersection(Node):
    ...
    def enter_input_node(self, entity, **kwargs):
        ...
        if type(self.graph) == nx.classes.graph.Graph:
            entity.output_route[int(self.simulator.simulator_time)] = self.name
        else:
            ...
            try:
                ...
                if len(neighbors) > 1:
                    if random.random() >= self.jitter:
                        ...
                        # change link travel_time_adj of opposite link to high (only in directed (city) graphs)
                        if type(self.graph) != nx.classes.graph.Graph:
                            for u, v in self.graph.in_edges(self.name):
                                self.graph[u][v][0]['travel_time'] += 60
                                try:
                                    self.graph[u][v][1]['travel_time'] += 60
                                except:
                                    pass
                        ...
                        entity.route_planned = nx.shortest_path(self.graph, next_node, entity.escape_node, 'travel_time')
                else:
                    next_node = entity.route_planned[0]
